...
export function compareMidiBuffers(referenceBuffer, candidateBuffer, options = {}) {
  const referenceArrayBuffer = toArrayBuffer(referenceBuffer);
  const candidateArrayBuffer = toArrayBuffer(candidateBuffer);
  const referenceSong = parseMidi(referenceArrayBuffer);
  const candidateSong = parseMidi(candidateArrayBuffer);

  return compareSongs(referenceSong, candidateSong, {
    ...DEFAULT_COMPARE_OPTIONS,
    ...options,
    referenceTempo: readTempoSummary(referenceArrayBuffer) || defaultTempoSummary(),
    candidateTempo: readTempoSummary(candidateArrayBuffer) || defaultTempoSummary(),
  });
}
...
export function compareSongs(referenceSong, candidateSong, options = {}) {
  ...
  return {
    files: {
      reference: settings.referenceName || "reference",
      candidate: settings.candidateName || "candidate",
    },
    settings: {
...
export function formatSetReport({ reference, reports }) {
  const rows = reports
    .map(({ label, report }) => {
